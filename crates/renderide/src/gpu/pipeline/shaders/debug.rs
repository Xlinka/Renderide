...
//!
//! Single-target normal and UV debug shaders are composed from `wgsl_modules/` via [naga_oil] in
//! `build.rs` (see `uniform_ring`, `color_util`, and entry modules). MRT and overlay variants stay
//! inline until migrated.
//!
//! [naga_oil]: https://docs.rs/naga_oil/

/// Normal debug shader: colors surfaces by smooth world normal (single color target).
pub(crate) const NORMAL_SHADER_SRC: &str =
    include_str!(concat!(env!("OUT_DIR"), "/normal_debug.wgsl"));

/// UV debug shader: colors surfaces using HSV hue-saturation from UV coordinates (single color target).
pub(crate) const UV_DEBUG_SHADER_SRC: &str =
    include_str!(concat!(env!("OUT_DIR"), "/uv_debug.wgsl"));
